...
void Configuration::ParseNumber(std::string Key, T& Var) {
    ...
    // Look for the colon after the key name, then skip whitespace to find the actual value
    ...
    // Find the end of the number (stop at comma, newline, or closing brace)
    ...
    // Use appropriate parsing based on type
    if constexpr (std::is_integral<T>::value) {
        // For integer types, use stoi to avoid floating point conversion
        T oldValue = Var;
        Var = static_cast<T>(std::stoi(numberStr));
    }
    else {
        // For floating point types, use stof
...
void Configuration::Load(GolfOverlay* Overlay) {
